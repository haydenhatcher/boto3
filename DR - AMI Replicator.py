...
# Disabled this to use the logic below
...
# Get all self made AMIs
    ...
    for image in images:
        # print("ALL IMAGES ALL TIME. Image Name:{name} ID:{id}'".format(name=image.name,id=image.id))

        image_date = parser.parse(image.creation_date)
        # print("ALL IMAGES WITH TIME. Image Name:{name} ID:{id} DATE:{date}'".format(name=image.name,id=image.id,date=image.creation_date))
        # print(image_date.date())
        # print ((datetime.datetime.today()-datetime.timedelta(160)).date())
        # Copy ONLY today's images
        #if image_date.date() == (datetime.datetime.today()).date():

        # To Copy previous day images
        if image_date.date() > (datetime.datetime.today()-datetime.timedelta(10)).date():
            ...
            for awsRegion in globalVars['destRegions']:
                ...
                # Set AMI Tags, inherit from origin AMI if any exist
                    amitags=image.tags
                    if amitags == None:                    
                        amitags = [{'Key': 'OriginTags', 'Value': 'No Tags existed on AMI in origin region'}]
                    ...
                    print('endresult', amitags)
                    print(new_ami['ImageId'])

                    # Add tag to the AMI
                    destEC2Client.create_tags( Resources=[new_ami['ImageId']],
                                                Tags = amitags
                                                )
                else:
